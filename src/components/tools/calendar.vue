<template>
    <div class="calendar calendarCenter">
        <div class="calendar-tools">
            ...
            <div class="calendar-info"  @click="focus" >
                <!-- {{monthString}} -->
                <div class="month">
                    <div class="month-inner" :style="{'top':-(this.month*20)+'px'}">
                      <span v-for="m in months">{{year}}年{{m}}{{day}}日</span>
                    </div>
        ...
        <table cellpadding="5"  v-show="changeShowCalendar">
        <thead>
            <tr>
               <td v-for="(item, index) in weeks" :class="{weekend: index === 0 || index === 6}">
            {{item}}
          </td>
            </tr>
        </thead>
...
export default {
  ...
  activated(){
//    alert(this.isFirst);
    if(this.$route.query.date=="date"||this.$route.query.flag=="11")
    {
      this.init();
    }
  },
        ...
        select(k1, k2, e) {
          ...
          this.changeShowCalendar = !this.changeShowCalendar;
            ...
            if (this.range) {
                ...
                this.today = [k1, k2]
                this.$emit('getValue',[this.year,this.zero?this.zeroPad(this.month + 1):this.month + 1,this.zero?this.zeroPad(this.days[k1][k2].day):this.days[k1][k2].day])
            }
        },
        changeYear(){
            if(this.yearsShow){
...
.calendar-years>span.active{
  ...
  /*设置日期抬头样式*/
.calendar .smallCircle{
  width: 25px;
  height: 25px;
  line-height: 25px;
  display: inline-block;
  border-radius: 50%;
  border: 1px solid #8096A1;
  text-align: center;
}
.calendar .smallDown{
  width: 20px;
