<template>
   <div class="cardWrap" :style="{height:divHeight+'px'}" ref="nav">
     ...
     >
      <el-collapse v-model="activeNames" v-for="(item,index) in userLS" :key="index">
          <el-collapse-item :title=item.custName>
            <router-link :to="{path:'/seeCustomer',query:{id: item.custId,clintflag:clientflag}}">
              <div>
                <span class="tit1">潜在客户编号：</span>
                <span class="value1">{{item.custId}}</span>
              </div>
              <div>
                <span class="tit1">证件类型:</span>
                <span class="value1">{{item.identType}}</span>
              </div>
              <div>
                <span class="tit1">证件号码:</span>
                <span class="value1">{{item.identNo}}</span>
              </div>
            </router-link>
          </el-collapse-item>
      </el-collapse>
